...
def plot_seasonal_decomposition(axs, series, sd):
    ...
    axs[0].set_ylabel('Degrees', fontsize = 16)
    axs[1].plot(sd.trend, c='b')
    axs[1].set_title("Trend Component $T_t$")
    axs[1].set_ylabel('Degrees', fontsize = 16)
    axs[2].plot(sd.seasonal, c='b')
    axs[2].set_title("Seasonal Component $S_t$")
    axs[2].set_ylabel('Degrees', fontsize = 16)
    axs[3].plot(sd.resid, c='b')
    axs[3].set_title("Residual Component $R_t$")
    axs[3].set_ylabel('Degrees', fontsize = 16)
    axs[3].set_xlabel('Date', fontsize = 16)
...
if __name__ == '__main__':
    # fig, axs = plt.subplots(2, figsize=(24, 8), dpi = 150)
    
    # axs[0].plot(den_2015['2015'], c='b')
    # axs[0].set_title('Daily Denver Temperature')
    # axs[1].plot(san['2015'], c='r')
    # axs[1].set_title('Daily Austin Temperature')
    # axs[1].set_ylabel("Degrees Fahrenheit", fontsize = 18)
    # axs[1].set_xlabel("Date", fontsize = 18)
    # axs[0].set_ylabel("Degrees Fahrenheit", fontsize = 18)
    # plt.show()

    fig, axs = plt.subplots(4, figsize = (24, 8), dpi = 150)
